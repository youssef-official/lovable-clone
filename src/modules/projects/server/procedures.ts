    ...
    .mutation(async ({ input, ctx }) => {
        ...
        if (input.provider === 'vercel') {
            ...
            try {
                 ...
                 // For custom domains, we only support it if using the default credentials (system owned) or if the user wants to map it.
                 // If the user provides their own credentials, they might not want 'youssef-elsayed.tech' mapping or might not have permissions for that zone.
                 // So we skip custom domain mapping if using custom credentials, UNLESS we assume they want to use their own domain?
                 // For now, let's keep the original logic ONLY if no custom creds are provided (or if the subdomain implies it).
                 // Actually, simpler: if they provide their own CF account, they get the `*.pages.dev` URL. We don't mess with custom domains on their account.
